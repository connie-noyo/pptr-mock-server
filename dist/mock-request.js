 ...
 * @hideconstructor
 */
function MockRequest(handlers, baseApiUrl = '', timeScaleFactor = 1) {
  const handler = (method, endpoint, status, options = {}) => {
    if (!endpoint.startsWith('http://') && !endpoint.startsWith('https://')) {
      endpoint = baseApiUrl + endpoint;
    }

    if ((0, _isNumber.default)(options.delay)) {
  ...
  this.on = (method, endpoint, status, response) => handler(method, endpoint, status, response);

  const methods = ['get', 'post', 'put', 'delete'];
  methods.forEach(method => {
    this[method] = (endpoint, status, options) => handler(method, endpoint, status, options);
  });
}
/**
